...
jobs:
  ci:
    uses: zebradil/nix-ci/.github/workflows/ci.yaml@v1.1.0
    with:
...
jobs:
  update:
    uses: zebradil/nix-ci/.github/workflows/update-pr.yaml@v1.1.0
    with:
      auto-merge: true
      diff-targets: |
...
<!-- x-release-please-start-version -->
```yaml
      - uses: zebradil/nix-ci/.github/actions/setup-nix@v1.1.0
        with:
          signing-key: ${{ secrets.CACHE_SIGNING_KEY }}
      - id: build
        uses: zebradil/nix-ci/.github/actions/build@v1.1.0
        with:
...
Current release: `v1.1.0` <!-- x-release-please-version -->
